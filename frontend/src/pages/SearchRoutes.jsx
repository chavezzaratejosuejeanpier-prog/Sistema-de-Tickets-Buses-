...
function AutocompleteField({ label, value, onChange, placeholder, exclude = '', onSwap }) {
  ...
  return (
    <div className="flex-1 relative">
      <label className="text-xs font-bold tracking-widest text-gray-500 uppercase mb-1.5 block">{label}</label>
      <div className="relative">
        ...
        <input
          ...
          autoComplete="off"
          className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium text-primary placeholder:text-gray-400 focus:outline-none focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/10 transition"
        />
      </div>
      {showList && (
        <ul className="absolute z-20 mt-2 w-full bg-white border border-gray-100 rounded-xl shadow-xl overflow-hidden py-1">
          {filtered.map(c => (
            <li key={c}>
              <button
                type="button"
...
export default function SearchRoutes() {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-3xl md:text-4xl font-extrabold text-primary tracking-tight">Buscar Viajes</h1>
        <p className="text-gray-500 mt-2 text-sm md:text-base">Elige origen, destino y fecha. Resultados en tiempo real con asientos disponibles.</p>
      </div>

      {/* Form Card */}
      <form onSubmit={buscar} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-7">
        <div className="flex flex-col md:flex-row gap-4 items-stretch md:items-end">
          ...
          <button
            ...
            title="Intercambiar"
            className="self-center md:self-end shrink-0 w-10 h-10 rounded-full bg-white border border-gray-200 shadow-sm flex items-center justify-center text-primary hover:bg-primary hover:text-white hover:border-primary transition-colors"
          >
          ...
          <div className="flex-1 md:max-w-[200px]">
            <label className="text-xs font-bold tracking-widest text-gray-500 uppercase mb-1.5 block">Fecha</label>
            <div className="relative">
              <input
                type="date"
                value={fecha}
                ...
                className="w-full pl-4 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium text-primary focus:outline-none focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/10 transition"
              />
            </div>
            <p className="text-[11px] text-gray-400 mt-1.5">Opcional — filtra por día</p>
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          <button type="button" onClick={limpiar} className="px-6 py-2.5 rounded-xl font-semibold text-sm border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">
            Limpiar
          </button>
      ...
      <div className="mt-6">
        {!hasSearched ? (
          <div className="bg-primary/5 border border-dashed border-primary/20 rounded-2xl p-8 text-center">
            <p className="text-sm text-gray-600">Ingresa <span className="font-bold text-primary">origen</span> y <span className="font-bold text-primary">destino</span> y presiona <span className="font-bold">Buscar</span>. Usa el calendario para filtrar por fecha.</p>
            <div className="mt-4 flex flex-wrap justify-center gap-2">
              {['Lima → Cusco', 'Lima → Arequipa', 'Trujillo → Lima'].map(s => (
                <button key={s} onClick={() => { const [o,d]=s.split(' → '); setOrigen(o); setDestino(d) }} className="text-xs bg-white border border-gray-200 px-3 py-1.5 rounded-full hover:border-primary hover:text-primary transition-colors">{s}</button>
              ))}
            </div>
          </div>
        ) : loading ? (
          ...
          <>
            <p className="text-xs font-bold tracking-widest text-gray-400 uppercase mb-3">{resultados.length} resultado(s) {fecha && `• ${new Date(fecha).toLocaleDateString()}`}</p>
            <div className="grid gap-4">
              {resultados.map(r => (
                <div key={r.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all p-5 flex flex-col sm:flex-row justify-between gap-4">
                  <div className="min-w-0">
                    <p className="font-extrabold text-primary text-base flex items-center gap-2">
                  ...
                  <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-3 sm:text-right shrink-0">
                    <p className="text-2xl font-extrabold text-primary">S/ {Number(r.precio_base).toFixed(2)}</p>
